import { UserState, MetricScores, MetricID, ReportContent, MaturityLevel, ScaleStage, ActionItem } from '@/lib/quiz/types';
import { getRiskScore } from '@/lib/quiz/engine/scoring';
import type { DiagnosticResult, ManagementPattern } from '@/lib/quiz/types';
...
export const generateReportText = (
  ...
  if (!PATTERN_COPY[patternKey]) {
    console.error('[generateReportText] Pattern key missing in PATTERN_COPY, falling back to CENTRALIZED', { patternKey, patternId: pattern.id });
  }
