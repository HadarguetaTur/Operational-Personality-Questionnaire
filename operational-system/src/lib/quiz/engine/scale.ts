import { UserState, ScaleStage } from '@/lib/quiz/types';
...
export function inferScale(state: UserState): ScaleStage {
  ...
  console.error('[inferScale] No scaleContext on state, defaulting to small_team', {
    hasAnswers: Object.keys(state.answers ?? {}).length,
    leadId: state.leadId
  });
  return 'small_team';
}
