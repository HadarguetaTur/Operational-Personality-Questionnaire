...
את המערכת הדיגיטלית של הדר תורגמן — מומחית לאוטומציות לעסקים קטנים.
זוהי ההודעה הראשונה.
...
2. מטרה: "אני כאן כדי לענות על שאלות ולבדוק ביחד האם הדר יכולה לסייע לך, לפני שמתקדמות לפגישה."
...
## כיצד להגיב אחרי הפתיחה (בסדר עדיפות)
- ביקשה פגישה/קישור מפורשות → action: book_meeting, state: booking מיד (ללא פתיחה)
...
const STAGE_FALLBACKS: Record<string, StageFallback> = {
  initial:    { reply: 'היי 😊 אני המערכת הדיגיטלית של הדר תורגמן, שמלווה פניות נכנסות. אני כאן כדי לענות על שאלות ולבדוק ביחד האם הדר יכולה לסייע לך, לפני שמתקדמות לפגישה. ספרי לי — מה קורה עכשיו עם הפניות שמגיעות אלייך?', state: 'discovery' },
