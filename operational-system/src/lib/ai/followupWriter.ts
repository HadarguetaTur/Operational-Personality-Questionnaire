...
export async function runFollowupWriter(input: {
  ...
  const touchBrief =
    input.touch === 1
      ? 'זו תזכורת ראשונה, כ-3 שעות אחרי שהשיחה דעכה בלי שנקבעה פגישה. קצרה, חמה, בלי לחץ, מזמינה להמשיך מאיפה שעצרנו.'
  ...
  const systemPrompt = [
    'את כותבת הודעת תזכורת אחת קצרה בוואטסאפ, בקול של הדר, ללקוחה שהשיחה איתה נקטעה בלי שנקבעה פגישה.',
    ...
    touchBrief,
    factLines.length > 0 ? `מה ידוע על הלקוחה:\n${factLines.join('\n')}` : '',
