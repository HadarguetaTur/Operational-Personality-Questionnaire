...
import { LandingCopy, getPaymentUrl } from '@/config/landingCopy';
...
export const HeroSection: React.FC<{
  ...
  return (
    <section className="relative min-h-[50vh] sm:min-h-[55vh] md:min-h-[65vh] flex flex-col justify-center py-12 md:py-16 landing-gradient-bg">
      <HeroGraphics />
      <div className="relative z-10 w-full max-w-[var(--max-content)] mx-auto px-6 md:px-8">
        <div className="max-w-[55ch] mx-auto text-right landing-hero-stagger" dir="rtl">
          ...
          <div
            ...
            style={{
              background:
                'linear-gradient(90deg, transparent, rgba(255,255,255,0.5) 20%, rgba(94,234,212,0.7) 50%, rgba(255,255,255,0.5) 80%, transparent)',
            }}
            aria-hidden
          />
          <p className="text-xl md:text-2xl text-white/95 leading-snug mb-5">{copy.subheadline}</p>
          <p className="text-lg font-medium text-white/95 mb-3">{copy.microDiagnosisHeadline}</p>
          <ul className="space-y-2.5 mb-8 max-w-prose mr-0" role="list">
            {copy.microDiagnosisBullets.slice(0, 3).map((b, i) => (
              <li key={i} className="flex gap-3 items-start text-white/95 text-base md:text-[17px]">
                <span
                  className="shrink-0 w-5 h-5 rounded border-2 border-white/50 flex items-center justify-center mt-0.5 text-white text-xs font-bold"
                  aria-hidden
                >
                  ✓
                </span>
