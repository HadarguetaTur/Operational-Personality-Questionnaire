...
const inputBase =
  'w-full rounded-xl px-4 py-3.5 text-white text-[15px] md:text-base transition-colors duration-200 placeholder:text-white/28 focus:outline-none';

const landingInputClass =
  `${inputBase} border border-white/12 bg-black/25 focus:border-[var(--landing-primary)]/55 focus:ring-2 focus:ring-[var(--landing-primary)]/25`;

const homeInputClass =
  `${inputBase} border border-white/[0.08] bg-[#0a0f18]/65 focus:border-teal-500/45 focus:ring-2 focus:ring-teal-400/18`;
...
export const ContactSection: React.FC<ContactSectionProps> = ({ variant = 'landing' }) => {
  ...
  const sectionClass =
    variant === 'home'
      ? 'relative py-24 md:py-32 px-5 md:px-8 overflow-hidden bg-gradient-to-b from-[#0f1729] to-[#0c1220] scroll-mt-20'
      ...
      {variant === 'home' ? (
        <>
          <div className="absolute inset-0 pointer-events-none" aria-hidden>
            <div className="absolute top-1/4 left-1/2 -translate-x-1/2 w-[min(720px,90vw)] h-[280px] bg-teal-500/[0.07] rounded-full blur-[100px]" />
            <div
          ...
          <div dir="rtl">
            {variant === 'home' ? (
              <div className="text-center mb-12 md:mb-14">
                ...
                <a
                  ...
                  className="mt-6 inline-flex min-h-[46px] items-center justify-center gap-2 rounded-xl border border-emerald-400/30 bg-emerald-500/12 px-6 text-sm font-semibold text-emerald-200 transition-all duration-200 hover:border-emerald-300/55 hover:bg-emerald-500/20 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-300/60 focus-visible:ring-offset-2 focus-visible:ring-offset-[#0c1220]"
                >
            ...
            <div
              className={`mx-auto rounded-2xl border ${
                variant === 'home'
                  ? 'max-w-2xl border-white/[0.06] bg-white/[0.02] backdrop-blur-sm shadow-[0_0_60px_-20px_rgba(20,184,166,0.15)] p-8 md:p-10'
            ...
            >
              {formState === 'success' ? (
                ...
                >
                  <div className="w-14 h-14 mx-auto mb-4 rounded-full bg-gradient-to-br from-teal-500/25 to-emerald-600/15 border border-teal-400/30 flex items-center justify-center">
                    <svg className="w-7 h-7 text-teal-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.2}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                    </svg>
                  </div>
                  <p className="font-semibold text-white mb-2 text-lg">ההודעה נשלחה בהצלחה</p>
                  <p className="text-sm text-white/55">ניצור קשר בהקדם.</p>
                </div>
              ) : (
                <form onSubmit={handleSubmit} className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      <label htmlFor="contact-name" className="block text-sm font-medium text-white/65 mb-2">
                        שם מלא <span className="text-teal-400/90">*</span>
                      </label>
                      <input
                        id="contact-name"
                        name="name"
                    ...
                    <div>
                      <label htmlFor="contact-email" className="block text-sm font-medium text-white/65 mb-2">
                        אימייל <span className="text-teal-400/90">*</span>
                      </label>
                      <input
                        id="contact-email"
                        name="email"
                  ...
                  <div>
                    <label htmlFor="contact-phone" className="block text-sm font-medium text-white/65 mb-2">
                      טלפון <span className="text-white/35 font-normal text-xs mr-1">(אופציונלי)</span>
                    </label>
                    <input
                      id="contact-phone"
                      name="phone"
                  ...
                  <div>
                    <label htmlFor="contact-message" className="block text-sm font-medium text-white/65 mb-2">
                      הודעה <span className="text-teal-400/90">*</span>
                    </label>
                    <textarea
                      id="contact-message"
                      name="message"
                  ...
                  {formState === 'error' && errorMessage ? (
                    <div
                      role="alert"
                      className="rounded-xl border border-red-500/35 bg-red-950/[0.25] px-4 py-3.5 text-red-100/95 text-sm leading-relaxed"
                    >
                      {errorMessage}
                    </div>
                  ) : null}
