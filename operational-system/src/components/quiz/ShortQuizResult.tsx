        ...
        <section className="border-t border-[var(--qa-border-light)] pt-8">
          ...
          <div className="mb-6 p-5 rounded-[14px] bg-[var(--qa-surface)] border border-[var(--qa-border)]">
            <p className="text-[17px] font-bold text-[var(--qa-text-primary)] mb-1">
              שיחת אבחון — חינם. 30 דקות. תצאי עם תוכנית.
            </p>
            <p className="text-[14px] text-[var(--qa-text-muted)] mb-4">
              לא הרצאה ולא מכירה. נסתכל ביחד על העסק שלך ונחליט מה פותרים ראשון.
            </p>
            <ul className="flex flex-col gap-2">
              {[
                'מזהות את הצוואר הבקבוק המרכזי שלך',
                'יוצאת עם סדר עדיפויות ברור ל-30 הימים הקרובים',
                '3 פעולות קונקרטיות לשבוע הראשון — מותאמות לעסק שלך',
              ].map((item) => (
...
function WhatsAppCTA({ whatsappText }: { firstName: string; whatsappText: string }) {
  return (
    ...
    >
      ...
      שמרי מקום בשיחת האבחון — בוואטסאפ ←
    </a>
  );
}
