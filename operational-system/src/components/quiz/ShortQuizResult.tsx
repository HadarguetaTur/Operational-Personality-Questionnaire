...
import type { ResultType, FixStep } from '@/config/shortQuizResults';
      ...
      <div className="max-w-[600px] mx-auto text-right">
        ...
        {isNew && (
          <div className="mb-8 p-4 rounded-[12px] border border-[var(--qa-accent)] border-opacity-30 bg-[var(--qa-accent-soft)] text-[14px] text-[var(--qa-text-secondary)] leading-relaxed">
            קיבלנו. ראי למטה מה אפשר לבדוק קודם.
          </div>
        )}
        ...
        <div className="mb-6">
          <h1 className="text-[26px] md:text-[30px] font-bold text-[var(--qa-text-primary)] mb-2 leading-snug">
            {firstName ? `${firstName} — ${content.headline}` : content.headline}
          </h1>
        ...
        <section className="border-t border-[var(--qa-border-light)] pt-8">
          ...
          <div className="mb-6 p-5 rounded-[14px] bg-[var(--qa-surface)] border border-[var(--qa-border)]">
            <p className="text-[17px] font-bold text-[var(--qa-text-primary)] mb-1">
              שיחת התאמה קצרה בוואטסאפ
            </p>
            <p className="text-[14px] text-[var(--qa-text-muted)] mb-4">
              נבדוק אם יש כאן תהליך שכדאי לסדר עכשיו, ומה הצעד הנכון לפני שמשקיעים בפגישת אפיון.
            </p>
            <ul className="flex flex-col gap-2">
              {[
                'נבין אם המספרים מצביעים על בעיה ששווה לטפל בה עכשיו',
                'נזהה מה כנראה תוקע את התהליך לפני שמציעים פתרון',
                'אם יש התאמה, אפשר להמשיך לפגישת אפיון ממוקדת בעלות ₪350',
              ].map((item) => (
                <li key={item} className="flex gap-2 items-start text-[14px] text-[var(--qa-text-secondary)]">
                  <span className="text-[var(--qa-accent)] font-bold mt-px">✓</span>
                  {item}
                </li>
              ))}
            </ul>
