...
export function QuizLeadForm({ resultType, answerInputs }: QuizLeadFormProps) {
  ...
  return (
    <div
      className="flex flex-col min-h-screen justify-center bg-[var(--qa-bg)] text-[var(--qa-text-primary)] px-6 md:px-8 py-10"
      dir="rtl"
    >
      <section className="w-full max-w-[480px] mx-auto">
        <p className="text-[13px] font-medium text-[var(--qa-accent)] mb-2 opacity-80">
          רגע לפני התוצאה
        </p>
        <h1 className="text-[24px] md:text-[28px] font-bold leading-snug mb-3">
          לאן לשלוח לך את התמונה המלאה?
        </h1>
        <p className="text-[15px] text-[var(--qa-text-secondary)] leading-relaxed mb-7">
        ...
        <form onSubmit={handleSubmit} className="space-y-5" noValidate>
          <div>
            ...
            <input
              ...
              className={`w-full h-12 px-4 rounded-xl border bg-[var(--qa-surface)] text-[var(--qa-text-primary)] placeholder-[var(--qa-text-muted)] focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--qa-accent)] focus-visible:border-transparent transition-colors ${nameHint ? 'border-rose-400' : 'border-[var(--qa-border)]'}`}
              dir="rtl"
          ...
          <div>
            ...
            <input
              ...
              className={`w-full h-12 px-4 rounded-xl border bg-[var(--qa-surface)] text-[var(--qa-text-primary)] placeholder-[var(--qa-text-muted)] focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--qa-accent)] focus-visible:border-transparent transition-colors ${phoneHint ? 'border-rose-400' : 'border-[var(--qa-border)]'}`}
              dir="ltr"
              autoComplete="tel"
              required
              aria-required="true"
          ...
          <div>
            ...
            <input
              ...
              className={`w-full h-12 px-4 rounded-xl border bg-[var(--qa-surface)] text-[var(--qa-text-primary)] placeholder-[var(--qa-text-muted)] focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--qa-accent)] focus-visible:border-transparent transition-colors ${emailHint ? 'border-rose-400' : 'border-[var(--qa-border)]'}`}
              dir="ltr"
              autoComplete="email"
          ...
          <button
            ...
            className="mt-1 w-full h-14 rounded-xl bg-[var(--qa-accent)] text-white text-[17px] font-semibold transition-all hover:opacity-90 active:scale-[0.99] disabled:opacity-60 disabled:cursor-not-allowed"
          >
            {submitting ? 'רגע, מכינה לך את התמונה...' : 'לתוצאה שלי ←'}
          </button>

          <p className="text-[12px] text-[var(--qa-text-muted)] text-center leading-relaxed">
            הפרטים נשמרים אצל הדר בלבד, ולא מועברים לאף אחד.
          </p>
        </form>
      </section>
    </div>
