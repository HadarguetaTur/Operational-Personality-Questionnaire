...
import { UserState, DiagnosticResult, ReportContent, MetricID, ActionItem, MaturityLevel, ScaleStage } from '../types';
import { normalizeScores, getRiskScore } from '../engine/scoring';
import { determinePattern, MANAGEMENT_PATTERNS } from '../engine/patterns';
...
import { resultSnapshotSchema, aiDiagnosisSchema } from '../lib/schemas';
import { finalizeDiagnostic } from '../lib/finalizeDiagnostic';
import { requestAiDiagnosis } from '../lib/aiDiagnosis';
import type { ValidatedAiDiagnosis } from '../lib/schemas';
...
export function useReport(token: string | undefined): UseReportResult {
  ...
  useEffect(() => {
    ...
    const fetchExistingReport = async () => {
      // Schema-tolerant fetch: try the columns added in Sprint 3 (ai_diagnosis*)
      // and Sprint 4 (report_pdf_*). If the DB hasn't been migrated yet,
      // PostgREST returns 400 "column does not exist" — fall back to the
      // baseline columns so the report still renders.
      const COLUMNS_FULL =
      ...
      // Retry with exponential backoff if snapshot not ready yet (race with finalize step)
      // Total wait: 800ms + 1500ms + 3000ms ≈ 5.3s across 3 retries
      ...
      // Persona-specific landing in Next.js operational-system is reachable via
      // the explicit CTA button in FinalReport. We no longer auto-redirect — the
      // user reads the report at their own pace and clicks through manually.
      ...
      // ── Hydrate AI diagnosis ──────────────────────────────────────
      // If already cached on the row, surface it immediately. Otherwise,
      // kick off background generation. The base report renders regardless.
      ...
      if (cachedParse?.success) {
        ...
        setAiStatus('loading');
        // Fire-and-forget: do not block UI on AI completion.
        requestAiDiagnosis(leadId, token).then((res) => {
          if (cancelled) return;
          if (res.diagnosis) {
    ...
    const generateNewReport = async () => {
      ...
      if (!savedState) {
        ...
        setSaveError('לא הצלחנו למצוא את התשובות שלך. אם מילאת את השאלון לאחרונה, נסי לרענן או פני אלינו לתמיכה.');
        setFetchStatus('error');
        return;
      }
      ...
      const resultSnapshot = {
        ...
        // Stored so the AI diagnosis Edge Function can rebuild evidence prompts
        // from DB without depending on client state being still around.
      ...
      // The email goes out as part of the background pipeline now, so we don't
      // know synchronously if it failed. Older deployments may still return
      // 'failed' inline — surface the banner in that case.
      if (finalize.pipelineStatus === 'failed') {
        setEmailWarning(true);
      }
      await new Promise((r) => setTimeout(r, 300));
      navigate(`/result/${finalize.reportToken}`, { replace: true });
    };
  ...
  return { result, reportData, saveError, fetchStatus, emailWarning, aiDiagnosis, aiStatus, reportPdfUrl, reportPdfStatus };
}
